...
from openedx.features.course_experience.views.welcome_message import (
    dismiss_welcome_message as disable_welcome_message, PREFERENCE_KEY, WelcomeMessageFragmentView
)
...
def dismiss_welcome_message(request):
    ...
    if len(request.data) > 1:
        raise ParseError("Only 'course_id' is expected.")

    try:
        disable_welcome_message(request, course_id)
        return Response({'message': 'Welcome message successfully dismissed.'})
    except Exception:
